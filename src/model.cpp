...
void model::send_prompt( std::string_view prompt, const std::vector<std::string>& path ) {
  ...
  req.request( );
  int i = 0;
  while ( this->model_stream.tool_call ) {
    this->handle_tool_call_stream( &this->model_stream );
    i++;
  }
...
void model::handle_tool_call_stream( stream_context* model_context ) {
  if ( model_context->tool_name == "web_search" ) {
    ...
    model_context->conversation->store_tool_result_str( model_context->tool_name, model_context->tool_id, trimmed_web_result_in_str );
...
void model::initialize( ) {
  ...
  req.set_body_write_cb( model_stream_speak_callback );
  req.set_body_cb_data( &this->model_stream );
  req.set_custom_option_list( "Content-Type", "application/json" );
  req.set_custom_options( );
  this->model_stream.buffer.reserve( 1024 * 1024 );
  this->model_stream.conversation = &this->conversation;
  this->model_stream.ctx = this;

  this->chat_context.response_buffer.reserve( 1024 * 1024 );

  web.req.initialize( );
  web.req.set_url( "https://api.tavily.com/search" );
  web.req.set_http_method_post( );
  web.req.set_custom_option_list( "Content-Type", "application/json" );
  std::string key = "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";
  web.req.set_custom_option_list( "Authorization", "Bearer " + key );
  web.req.set_body_write_cb( write_callback );
  web.req.set_body_cb_data( &web.buffer );
  web.req.set_custom_options( );

  client.req.initialize( );
  client.req.set_body_write_cb( write_callback );
  client.req.set_body_cb_data( &web.buffer );
  client.req.set_header_cb_data( &web.header );
  client.req.set_header_write_cb( header_callback );
  client.req.set_follow_redirect( true );
  ...
  chat_context.conversation = &conversation;
  chat_context.tool_call = false;
  chat_context.ctx = this;
  conversation.init( );
...
size_t model_stream_callback( char* ptr, size_t size, size_t nmemb, void* userdata ) { //this is for stream true, dumbass
  ...
  while ( ( end = context->buffer.find( '\n' ) ) != std::string::npos ) {
    ...
    if ( !context->think.error( ) ) {
      if ( context->thinking ) {
        std::cout << THINKING << context->think.get_string( ).value( ) << RESET;
      }
      else {
        context->thinking = true;
        std::cout << THINKING << "thinking: " << context->think.get_string( ).value( ) << RESET;
      }
    }
    else {
      auto tools = message["tool_calls"];
